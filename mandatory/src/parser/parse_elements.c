...
/*   Updated: 2025/11/05 22:31:23 by joao-alm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_ambient(char *line, t_scene *scene)
{
	...
	if (scene->ambient.set)
		return (ft_puterr("Ambient lighting already defined"));
...
int	parse_camera(char *line, t_scene *scene)
{
	...
	if (scene->camera.set)
		return (ft_puterr("Camera already defined"));
	params = ft_split(line, " \t\n\r");
...
int	parse_light(char *line, t_scene *scene)
{
	...
	if (scene->light.set)
		return (ft_puterr("Light already defined"));
	params = ft_split(line, " \t\n\r");
	if (count_split(params) != 4)
	{
		free_split(params);
